...
function validateHeroUrls(hero) {
  ...
  for (const field of urlFields) {
    ...
    if (val && !isSafeUrl(val)) {
      return {
        ok: false,
        msg: `${field} must be a valid http:// or https:// URL`,
      };
    }
  }
  return { ok: true };
...
exports.login = async (req, res) => {
  try {
    ...
    const DUMMY_HASH =
      "$2a$12$rBFqLJNTvLuIy0z3RkMCce3wfRSLq7X3YMp5HHvV.6Z/bDmSNu1uO";
...
exports.refreshToken = async (req, res) => {
  ...
  try {
    ...
    if (!updatedSession) {
      ...
      return res
        .status(401)
        .json({ message: "Security breach detected â€” please login again" });
    }
...
exports.enableTotp = async (req, res) => {
  try {
    ...
    if (!entry || entry.type !== "totp-setup")
      return res
        .status(400)
        .json({ message: "Setup session expired — please start over" });
    const totp = new OTPLib.TOTP({
      secret: OTPLib.Secret.fromBase32(entry.secret),
      digits: 6,
      period: 30,
      algorithm: "SHA1",
    });
    if (totp.validate({ token: code.replace(/\s/g, ""), window: 1 }) === null)
      return res
        .status(400)
        .json({ message: "Invalid code — check your authenticator app" });
    const admin = await Admin.findById(req.admin._id);
...
exports.disableTotp = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Disable TOTP failed" });
  }
};
...
exports.updateSection = async (req, res) => {
  try {
    ...
    if (!protocolCheck.ok)
      return res.status(422).json({ message: protocolCheck.msg });
    ...
    if (section === "projects" && Array.isArray(cleanPayload)) {
      for (const p of cleanPayload) {
        if (p.image) {
          ...
          if (!imgCheck.ok)
            return res.status(422).json({ message: imgCheck.msg });
        }
      }
    }
...
exports.changePassword = async (req, res) => {
  try {
    ...
    if (
      ...
      entry.adminId !== String(req.admin._id)
    )
      return res
        .status(400)
        .json({ message: "Verification code expired â€” request a new one" });
...
exports.forgotPassword = async (req, res) => {
  ...
  const GENERIC =
    "If that email matches an admin account, a reset code has been sent.";
  try {
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    if (!token || !code || !newPassword)
      return res
        .status(400)
        .json({ message: "Token, code, and new password are required" });
